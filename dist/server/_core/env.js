...
function validateJwtSecret(secret) {
    ...
    if (secret.length < MIN_SECRET_LENGTH) {
        console.warn(`⚠️  WARNING: JWT_SECRET should be at least ${MIN_SECRET_LENGTH} characters long. ` +
...
function validateDatabaseUrl(url) {
    ...
    try {
        ...
        if (!['postgresql:', 'postgres:'].includes(parsed.protocol)) {
            console.warn(`⚠️  WARNING: Database URL protocol '${parsed.protocol}' may not be supported. ` +
        ...
        if (url) {
            console.warn(`⚠️  WARNING: DATABASE_URL format issue. ` +
...
function initializeEnv() {
    ...
    try {
        ...
        if ((forgeApiUrl && !forgeApiKey) || (!forgeApiUrl && forgeApiKey)) {
            console.warn('⚠️  WARNING: Forge API is partially configured. ' +
