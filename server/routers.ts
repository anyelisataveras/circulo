    ...
    getUserDocuments: adminProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (input.userId) {
          conditions.push(
            or(
              eq(documents.uploadedByUserId, input.userId),
              isNull(documents.uploadedByUserId)
            )
          );
        }
  ...
  documents: router({
    list: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (input.applicationId !== undefined && input.applicationId !== null) {
          query = query.where(eq(documents.applicationId, input.applicationId)) as any;
        }
        
        if (input.documentType) {
          query = query.where(eq(documents.documentType, input.documentType)) as any;
        }

        if (input.category) {
          query = query.where(eq(documents.documentType, input.category)) as any;
        }
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await database.delete(documents).where(
          and(eq(documents.id, input.id), eq(documents.uploadedByUserId, ctx.user.id))
        );
  ...
  googleDrive: router({
    ...
    listFiles: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        driveService.setCredentials(
          ...
          user.googleTokenExpiry ? user.googleTokenExpiry.getTime() : undefined
        );
    ...
    importFile: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        let mimeType: string;
        let fileName: string = metadata.name || 'document';
