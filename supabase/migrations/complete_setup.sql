...
-- =====================================================
-- NOTA: Los buckets NO se pueden crear con SQL directo en Supabase.
-- Debes crearlos manualmente desde el Dashboard:
-- Storage → Create a new bucket
--
-- Buckets necesarios:
-- 1. 'documents' - Public, 50MB limit
-- 2. 'reports' - Public, 100MB limit  
-- 3. 'media' - Public, 10MB limit
-- 4. 'receipts' - Private, 20MB limit
-- 5. 'whatsapp' - Private, 16MB limit
--
-- O ejecuta este script después de crear los buckets manualmente:
-- https://supabase.com/dashboard/project/muywutvowctgvdtwavsw/storage/buckets

-- Los INSERTs están comentados porque requieren permisos especiales
-- Descomenta y ejecuta DESPUÉS de crear los buckets manualmente:
...
VALUES (
  ...
  true,
  10485760,
  ARRAY['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml', 'video/mp4', 'video/webm', 'audio/mpeg', 'audio/wav']
) ON CONFLICT (id) DO NOTHING;
...
VALUES (
  ...
  false,
  16777216,
  ARRAY['image/jpeg', 'image/png', 'image/webp', 'audio/ogg', 'audio/mpeg', 'audio/mp4', 'video/mp4', 'application/pdf']
) ON CONFLICT (id) DO NOTHING;
*/
...
SELECT 
  ...
  public.format_file_size(COALESCE(SUM((metadata->>'size')::BIGINT), 0)::BIGINT) AS total_size
...
-- Comentado porque requiere permisos especiales de owner
